...
export const api_v1_shoppingcart_list = createAsyncThunk("shoppingCarts/api_v1_shoppingcart_list", async payload => {
  const response = await apiService.api_v1_shoppingcart_list(payload);
  return response.data;
});
export const api_v1_shoppingcart_create = createAsyncThunk("shoppingCarts/api_v1_shoppingcart_create", async payload => {
  const response = await apiService.api_v1_shoppingcart_create(payload);
  return response.data;
});
export const api_v1_shoppingcart_retrieve = createAsyncThunk("shoppingCarts/api_v1_shoppingcart_retrieve", async payload => {
  const response = await apiService.api_v1_shoppingcart_retrieve(payload);
  return response.data;
});
export const api_v1_shoppingcart_update = createAsyncThunk("shoppingCarts/api_v1_shoppingcart_update", async payload => {
  const response = await apiService.api_v1_shoppingcart_update(payload);
  return response.data;
});
export const api_v1_shoppingcart_partial_update = createAsyncThunk("shoppingCarts/api_v1_shoppingcart_partial_update", async payload => {
  const response = await apiService.api_v1_shoppingcart_partial_update(payload);
  return response.data;
});
export const api_v1_shoppingcart_destroy = createAsyncThunk("shoppingCarts/api_v1_shoppingcart_destroy", async payload => {
...
const shoppingCartsSlice = createSlice({
  ...
  extraReducers: builder => {
    builder.addCase(api_v1_shoppingcart_list.pending, (state, action) => {
    ...
    }).addCase(api_v1_shoppingcart_partial_update.fulfilled, (state, action) => {
      if (state.api.loading === "pending") {
        state.entities = state.entities.map(record => record.id === action.payload.id ? action.payload : record);
        state.api.loading = "idle";
      }
